...
1. **Comentarios descriptivos**: Escribe comentarios claros antes de implementar funciones

```typescript
// Implementar función para buscar héroes por nombre que devuelva un array 
// filtrado de héroes y maneje correctamente búsquedas parciales (case insensitive)
function searchHeroesByName(heroes: Hero[], searchTerm: string): Hero[] {
  // Copilot sugerirá una implementación basada en este comentario
}
```

2. **Patrones consistentes**: Sigue patrones coherentes en todo el código

```typescript
// Patrón consistente para servicios
import { Hero } from '../models/hero.model';
...
// Ahora Copilot sugerirá patrones similares para nuevos servicios
export async function getVillains(): Promise<Villain[]> {
  // Copilot sugerirá una implementación con el mismo patrón
}
```

3. **Nombres descriptivos**: Usa nombres de variables y funciones claros

```typescript
// Nombres descriptivos facilitan sugerencias precisas
const topHeroesByPowerLevel = heroes
  .filter(hero => hero.powerLevel > 9000)
  .sort((a, b) => b.powerLevel - a.powerLevel)
  .slice(0, 5);
```

### Mejores Prácticas para Trabajar con Copilot

1. **Defina interfaces primero**: Ayuda a Copilot a entender la estructura de datos

```typescript
// Definir la interfaz primero
interface HeroStats {
  victories: number;
  defeats: number;
  powerLevel: number;
...
// Ahora Copilot puede generar mejores implementaciones
function calculateHeroEfficiency(stats: HeroStats): number {
  // Copilot sugerirá una implementación coherente con la interfaz
}
```

2. **Escribe firmas de funciones**: Proporciona tipos para parámetros y retorno

```typescript
// Proporcionar firma completa
function mergeHeroData(
  baseHero: Hero, 
  updates: Partial<Hero>, 
  options: { overwrite: boolean }
): Hero {
  // Copilot generará código que respete los tipos proporcionados
}
```

3. **Usa JSDoc para casos complejos**: Proporciona más contexto

```typescript
/**
 * Procesa un lote de actualizaciones de héroes en paralelo con límites de tasa
 * 
 * @param updates - Array de actualizaciones pendientes
 * @param batchSize - Número máximo de actualizaciones por lote
 * @param delayMs - Retraso entre lotes en milisegundos
 * @returns Objeto con resultados de éxito y error
 */
async function processBatchUpdates(
  updates: HeroUpdate[],
  batchSize = 10,
  delayMs = 1000
): Promise<{ successful: Hero[]; failed: Error[] }> {
  // Copilot generará una implementación que maneja lotes y errores
}
```
